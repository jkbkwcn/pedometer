...
// static SystemData &GetSystemDataInstance() {
...
void SystemData::init(void) {
  if(!mIsInitialized) {
    mData.insert(
        {DATA_STEPS, SystemParam(static_cast<uint32_t>(SYSTEM_STEPS_MIN), static_cast<uint32_t>(0), static_cast<uint32_t>(SYSTEM_STEPS_MAX))});
    mData.insert({DATA_STEPS, SystemParam(static_cast<uint32_t>(SYSTEM_TARGET_STEPS_MIN), static_cast<uint32_t>(0),
                                          static_cast<uint32_t>(SYSTEM_TARGET_STEPS_MAX))});
    mData.insert({DATA_STEPS, SystemParam(static_cast<uint8_t>(SYSTEM_HOURS_MIN), static_cast<uint8_t>(0), static_cast<uint8_t>(SYSTEM_HOURS_MAX))});
    mData.insert(
        {DATA_STEPS, SystemParam(static_cast<uint8_t>(SYSTEM_MINUTES_MIN), static_cast<uint8_t>(0), static_cast<uint8_t>(SYSTEM_MINUTES_MAX))});
    mData.insert(
        {DATA_STEPS, SystemParam(static_cast<uint8_t>(SYSTEM_SECONDS_MIN), static_cast<uint8_t>(0), static_cast<uint8_t>(SYSTEM_SECONDS_MAX))});
    mIsInitialized = true;
  } else {
